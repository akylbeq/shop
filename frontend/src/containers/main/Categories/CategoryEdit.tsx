...
import { CategoryEditing } from '../../../types.ts';
...
const CategoryEdit: React.FC<Props> = ({ closeModal, category }) => {
  const dispatch = useAppDispatch();
  const error = useAppSelector(selectCategoriesError);
  const [categoryMutation, setCategoryMutation] = useState<CategoryEditing>({
    _id: '',
    parentId: '',
    name: '',
    slug: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const response = await dispatch(editCategory(categoryMutation));
    if (editCategory.fulfilled.match(response)) {
